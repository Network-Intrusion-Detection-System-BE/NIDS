...
def scanAttacks_streaming(file_name):
    with pyshark.FileCapture(file_name) as capture :
        result_array = []
        n = 0
        for packet in capture:
...
def scanPCAP(request):
    if request.method == 'POST':
        ...
        # attack_array = StreamingHttpResponse(scanAttacks_streaming(file_name) ,content_type = 'text')
        # print("attack_array = " , attack_array.__dict__)
        # return attack_array
...
def scanLiveTraffic(request):
    asyncio.set_event_loop(loop)
    # print("Request Recieved for Live Traffic")
    result_array = StreamingHttpResponse(getLiveTraffic(), content_type='text/event-stream')
    print(result_array)
    return result_array
    # return render(request, 'NIDSApp/results2.html', {'event': result_array})
    # return render(request, 'NIDSApp/results1.html', {'packets': result_array})

def liveTraf(request):
    return render(request, 'NIDSApp/results2.html')
